
ElvCharacterDB = {
	["ChatEditHistory"] = {
		"/cw Kezt-Stormreaver nice mount man", -- [1]
		"/cw Kezt-Stormreaver my hunter just picked up the matching blue pterrodax from the un-instanced atal'dzar or whatever", -- [2]
		"/cw Kezt-Stormreaver check it out, it's a sweet combo and pterros have updraft, useful sometimes", -- [3]
		"/cw Kezt-Stormreaver 4 different colours iirc all flying around the uninstanced version of the dungeon, might take a bit to find a blue, iirc if you kill them the colours respawn randomly", -- [4]
		"/cw Acinonyx-Stormreaver such a nice mount", -- [5]
		"/cw Acinonyx-Stormreaver big too", -- [6]
		"/cw Acinonyx-Stormreaver ya zandalar, I got the rep but havent paid for it yet lol", -- [7]
		"/cw Fadedscourge-DemonSoul wheres that mount from?", -- [8]
		"/point", -- [9]
		"/cw Fadedscourge-DemonSoul oh nice ", -- [10]
		"/kiss", -- [11]
		"/raid warmode on?", -- [12]
		"/1 lets do lion's roar ppl", -- [13]
		"/1 need a tank then g2g", -- [14]
		"/raid rdy", -- [15]
		"/raid oh we had heals, guess they left", -- [16]
		"/raid no loot?", -- [17]
		"/tar darb", -- [18]
		"/dance", -- [19]
		"/s wtf is this", -- [20]
	},
	["ChatHistoryLog"] = {
		{
			"i remember back in the day where u could wipe to this", -- [1]
			"Arìe-Turalyon", -- [2]
			"", -- [3]
			"", -- [4]
			"Arìe-Turalyon", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			79, -- [11]
			"Player-3685-0708348D", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffaad372Arìe-Turalyon|r",
			[51] = 1538957878,
			[50] = "CHAT_MSG_INSTANCE_CHAT_LEADER",
		}, -- [1]
		{
			"Thanks", -- [1]
			"Golgurok-Trollbane", -- [2]
			"", -- [3]
			"", -- [4]
			"Golgurok-Trollbane", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			84, -- [11]
			"Player-1175-09BA67BD", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cff8787edGolgurok-Trollbane|r",
			[51] = 1538958078,
			[50] = "CHAT_MSG_INSTANCE_CHAT",
		}, -- [2]
		{
			"%s has earned the achievement |cffffff00|Hachievement:11846:Player-3693-09ED9289:1:10:7:18:4294967295:4294967295:4294967295:4294967295|h[Champions of Legionfall]|h|r!", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			244, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1538959452,
			[50] = "CHAT_MSG_GUILD_ACHIEVEMENT",
		}, -- [3]
		{
			"tons of caches by lost temple", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"1. General - Broken Shore", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Broken Shore", -- [9]
			0, -- [10]
			84, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539015737,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [4]
		{
			"how often do elites spawn?", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"1. General - Broken Shore", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Broken Shore", -- [9]
			0, -- [10]
			102, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539016206,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [5]
		{
			"so much of this zone is timegated cockblocking", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"1. General - Broken Shore", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Broken Shore", -- [9]
			0, -- [10]
			103, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539016222,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [6]
		{
			"you can do all the |cffffff00|Hachievement:11546:Player-3693-09ED9289:1:6:7:17:4294967295:4294967295:4294967295:4294967295|h[Breaching the Tomb]|h|r at once but there's time gates on some of the steps", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"1. General - Broken Shore", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Broken Shore", -- [9]
			0, -- [10]
			111, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539016364,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [7]
		{
			"like completing a legionfall mission", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"1. General - Broken Shore", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Broken Shore", -- [9]
			0, -- [10]
			112, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539016380,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [8]
		{
			"any rares up anywhere?", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"1. General - Broken Shore", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Broken Shore", -- [9]
			0, -- [10]
			113, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539016480,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [9]
		{
			"caches just respawned", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"1. General - Broken Shore", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Broken Shore", -- [9]
			0, -- [10]
			115, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539016739,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [10]
		{
			"ya", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"1. General - Broken Shore", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Broken Shore", -- [9]
			0, -- [10]
			21, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539046812,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [11]
		{
			"come to me", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"1. General - Broken Shore", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Broken Shore", -- [9]
			0, -- [10]
			22, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539046825,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [12]
		{
			"there ya go", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"1. General - Broken Shore", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Broken Shore", -- [9]
			0, -- [10]
			26, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539046848,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [13]
		{
			"%s has earned the achievement |cffffff00|Hachievement:11545:Player-3693-09ED9289:1:10:8:18:4294967295:4294967295:4294967295:4294967295|h[Legionfall Commander]|h|r!", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			31, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539046868,
			[50] = "CHAT_MSG_GUILD_ACHIEVEMENT",
		}, -- [14]
		{
			"%s has earned the achievement |cffffff00|Hachievement:11546:Player-3693-09ED9289:1:10:8:18:4294967295:4294967295:4294967295:4294967295|h[Breaching the Tomb]|h|r!", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			32, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539046868,
			[50] = "CHAT_MSG_GUILD_ACHIEVEMENT",
		}, -- [15]
		{
			"I just popped a ton, see the yellow dots on map under sentinax", -- [1]
			"Vahalia-ShatteredHalls", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Vahalia-ShatteredHalls", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			42, -- [11]
			"Player-155-06BE61B8", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffaad372Vahalia-ShatteredHalls|r",
			[51] = 1539046931,
			[50] = "CHAT_MSG_WHISPER_INFORM",
		}, -- [16]
		{
			"|cffffff00The Mugambala is under attack!|r", -- [1]
			"", -- [2]
			"", -- [3]
			"3. LocalDefense - Zuldazar", -- [4]
			"", -- [5]
			"", -- [6]
			22, -- [7]
			3, -- [8]
			"LocalDefense - Zuldazar", -- [9]
			0, -- [10]
			70, -- [11]
			false, -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "",
			[51] = 1539047520,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [17]
		{
			"%s has earned the achievement |cffffff00|Hachievement:12950:Player-3693-09ED9289:1:10:8:18:4294967295:4294967295:4294967295:4294967295|h[Zandalari Empire]|h|r!", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			83, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539047732,
			[50] = "CHAT_MSG_GUILD_ACHIEVEMENT",
		}, -- [18]
		{
			"where are the portals to Hyjal etc in Org?", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"2. Trade - City", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			2, -- [7]
			2, -- [8]
			"Trade - City", -- [9]
			0, -- [10]
			111, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539048534,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [19]
		{
			"like all the Cata portals?", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"2. Trade - City", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			2, -- [7]
			2, -- [8]
			"Trade - City", -- [9]
			0, -- [10]
			114, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539048650,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [20]
		{
			"vashjir etc?", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"2. Trade - City", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			2, -- [7]
			2, -- [8]
			"Trade - City", -- [9]
			0, -- [10]
			115, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539048653,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [21]
		{
			"know how to get to exodar?", -- [1]
			"Sunslayerr-LaughingSkull", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Sunslayerr-LaughingSkull", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			136, -- [11]
			"Player-101-082C6C5E", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Sunslayerr-LaughingSkull|r",
			[51] = 1539049164,
			[50] = "CHAT_MSG_WHISPER_INFORM",
		}, -- [22]
		{
			"there's a portal next to me for exodar, i'm trying to figure out how to get to bloodmyst lol", -- [1]
			"Sunslayerr-LaughingSkull", -- [2]
			"", -- [3]
			"", -- [4]
			"Sunslayerr-LaughingSkull", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			137, -- [11]
			"Player-101-082C6C5E", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Sunslayerr-LaughingSkull|r",
			[51] = 1539049198,
			[50] = "CHAT_MSG_WHISPER",
		}, -- [23]
		{
			"oh shit lol", -- [1]
			"Sunslayerr-LaughingSkull", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Sunslayerr-LaughingSkull", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			138, -- [11]
			"Player-101-082C6C5E", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Sunslayerr-LaughingSkull|r",
			[51] = 1539049215,
			[50] = "CHAT_MSG_WHISPER_INFORM",
		}, -- [24]
		{
			"there's a bridge from azuremyst to bloodmyst", -- [1]
			"Sunslayerr-LaughingSkull", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Sunslayerr-LaughingSkull", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			139, -- [11]
			"Player-101-082C6C5E", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Sunslayerr-LaughingSkull|r",
			[51] = 1539049228,
			[50] = "CHAT_MSG_WHISPER_INFORM",
		}, -- [25]
		{
			"doesnt show on the map of course", -- [1]
			"Sunslayerr-LaughingSkull", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Sunslayerr-LaughingSkull", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			140, -- [11]
			"Player-101-082C6C5E", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Sunslayerr-LaughingSkull|r",
			[51] = 1539049233,
			[50] = "CHAT_MSG_WHISPER_INFORM",
		}, -- [26]
		{
			"you doing waist of time?", -- [1]
			"Sunslayerr-LaughingSkull", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Sunslayerr-LaughingSkull", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			141, -- [11]
			"Player-101-082C6C5E", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Sunslayerr-LaughingSkull|r",
			[51] = 1539049247,
			[50] = "CHAT_MSG_WHISPER_INFORM",
		}, -- [27]
		{
			"im at rutherian righ tnow", -- [1]
			"Sunslayerr-LaughingSkull", -- [2]
			"", -- [3]
			"", -- [4]
			"Sunslayerr-LaughingSkull", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			142, -- [11]
			"Player-101-082C6C5E", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Sunslayerr-LaughingSkull|r",
			[51] = 1539049250,
			[50] = "CHAT_MSG_WHISPER",
		}, -- [28]
		{
			"naa, just trying to finish this battle pet achiev lol", -- [1]
			"Sunslayerr-LaughingSkull", -- [2]
			"", -- [3]
			"", -- [4]
			"Sunslayerr-LaughingSkull", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			145, -- [11]
			"Player-101-082C6C5E", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Sunslayerr-LaughingSkull|r",
			[51] = 1539049259,
			[50] = "CHAT_MSG_WHISPER",
		}, -- [29]
		{
			"take the exodar port then just follow the road north to the top of the map", -- [1]
			"Sunslayerr-LaughingSkull", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Sunslayerr-LaughingSkull", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			148, -- [11]
			"Player-101-082C6C5E", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Sunslayerr-LaughingSkull|r",
			[51] = 1539049280,
			[50] = "CHAT_MSG_WHISPER_INFORM",
		}, -- [30]
		{
			"ok thx", -- [1]
			"Sunslayerr-LaughingSkull", -- [2]
			"", -- [3]
			"", -- [4]
			"Sunslayerr-LaughingSkull", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			149, -- [11]
			"Player-101-082C6C5E", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Sunslayerr-LaughingSkull|r",
			[51] = 1539049286,
			[50] = "CHAT_MSG_WHISPER",
		}, -- [31]
		{
			"doesnt make any sense that we can use these alliance portals lol", -- [1]
			"Sunslayerr-LaughingSkull", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Sunslayerr-LaughingSkull", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			153, -- [11]
			"Player-101-082C6C5E", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Sunslayerr-LaughingSkull|r",
			[51] = 1539049307,
			[50] = "CHAT_MSG_WHISPER_INFORM",
		}, -- [32]
		{
			"yeah", -- [1]
			"Sunslayerr-LaughingSkull", -- [2]
			"", -- [3]
			"", -- [4]
			"Sunslayerr-LaughingSkull", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			154, -- [11]
			"Player-101-082C6C5E", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Sunslayerr-LaughingSkull|r",
			[51] = 1539049312,
			[50] = "CHAT_MSG_WHISPER",
		}, -- [33]
		{
			"fast way to get to Ironforge?", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"2. Trade - City", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			2, -- [7]
			2, -- [8]
			"Trade - City", -- [9]
			0, -- [10]
			182, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539049800,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [34]
		{
			"MAD MERCHANT UP IN DALARAN", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"2. Trade - City", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			2, -- [7]
			2, -- [8]
			"Trade - City", -- [9]
			0, -- [10]
			132, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539101362,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [35]
		{
			"MAD MERCHANT UP", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"2. Trade - City", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			2, -- [7]
			2, -- [8]
			"Trade - City", -- [9]
			0, -- [10]
			164, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539101667,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [36]
		{
			"MAD MERCHANT UP FOR 10 MORE MINUTES 42 45 IN LEGION DALARAN", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"2. Trade - City", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			2, -- [7]
			2, -- [8]
			"Trade - City", -- [9]
			0, -- [10]
			503, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539103727,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [37]
		{
			"%s has earned the achievement |cffffff00|Hachievement:12587:Player-3693-09ED9289:1:10:9:18:4294967295:4294967295:4294967295:4294967295|h[Unbound Monstrosities]|h|r!", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			134, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539106358,
			[50] = "CHAT_MSG_GUILD_ACHIEVEMENT",
		}, -- [38]
		{
			"|cffffff00The Sliver is under attack!|r", -- [1]
			"", -- [2]
			"", -- [3]
			"3. LocalDefense - Zuldazar", -- [4]
			"", -- [5]
			"", -- [6]
			22, -- [7]
			3, -- [8]
			"LocalDefense - Zuldazar", -- [9]
			0, -- [10]
			158, -- [11]
			false, -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "",
			[51] = 1539106446,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [39]
		{
			"does anyone see a quest called First Blood left side at Yogursa?", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"1. General - Arathi Highlands", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Arathi Highlands", -- [9]
			0, -- [10]
			193, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539106747,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [40]
		{
			"I see it on my map but dont see it in game", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"1. General - Arathi Highlands", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Arathi Highlands", -- [9]
			0, -- [10]
			195, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539106757,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [41]
		{
			"XD", -- [1]
			"Rimaa-Eredar", -- [2]
			"", -- [3]
			"1. General - Arathi Highlands", -- [4]
			"Rimaa-Eredar", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Arathi Highlands", -- [9]
			0, -- [10]
			271, -- [11]
			"Player-159-06EC7560", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cffaad372Rimaa-Eredar|r",
			[51] = 1539107654,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [42]
		{
			"hes here", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			284, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539107821,
			[50] = "CHAT_MSG_SAY",
		}, -- [43]
		{
			"fozruk up at me 59 28", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"1. General - Arathi Highlands", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Arathi Highlands", -- [9]
			0, -- [10]
			434, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539110591,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [44]
		{
			"rught by echo", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"1. General - Arathi Highlands", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Arathi Highlands", -- [9]
			0, -- [10]
			435, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539110600,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [45]
		{
			"LFM echo and fozruk", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"1. General - Arathi Highlands", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Arathi Highlands", -- [9]
			0, -- [10]
			436, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539110634,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [46]
		{
			"fozruk spawns just north of water ele so dont go far", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"1. General - Arathi Highlands", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Arathi Highlands", -- [9]
			0, -- [10]
			441, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539110753,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [47]
		{
			"fozruk up at water ele", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"1. General - Arathi Highlands", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Arathi Highlands", -- [9]
			0, -- [10]
			453, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539111992,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [48]
		{
			"|cffffff00Warbeast Kraal is under attack!|r", -- [1]
			"", -- [2]
			"", -- [3]
			"3. LocalDefense - Dazar'alor", -- [4]
			"", -- [5]
			"", -- [6]
			22, -- [7]
			3, -- [8]
			"LocalDefense - Dazar'alor", -- [9]
			0, -- [10]
			1902, -- [11]
			false, -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "",
			[51] = 1539189536,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [49]
		{
			"sup big chief two logs", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			32, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539193991,
			[50] = "CHAT_MSG_SAY",
		}, -- [50]
		{
			"ty", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			114, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539195619,
			[50] = "CHAT_MSG_SAY",
		}, -- [51]
		{
			"mad merchant in legion dalaran", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			115, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539195626,
			[50] = "CHAT_MSG_SAY",
		}, -- [52]
		{
			"2m", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			117, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539195636,
			[50] = "CHAT_MSG_SAY",
		}, -- [53]
		{
			"agreedo", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			119, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539195655,
			[50] = "CHAT_MSG_SAY",
		}, -- [54]
		{
			"zandalar flying mount is p cool tho", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			121, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539195664,
			[50] = "CHAT_MSG_SAY",
		}, -- [55]
		{
			"lol", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			123, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539195674,
			[50] = "CHAT_MSG_SAY",
		}, -- [56]
		{
			"|cffffff00|Hachievement:12950:Player-3693-09ED9289:1:10:8:18:4294967295:4294967295:4294967295:4294967295|h[Zandalari Empire]|h|r", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			126, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539195688,
			[50] = "CHAT_MSG_SAY",
		}, -- [57]
		{
			"glad to be done", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			127, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539195694,
			[50] = "CHAT_MSG_SAY",
		}, -- [58]
		{
			"nice I just ground it for the troll", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			132, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539195726,
			[50] = "CHAT_MSG_SAY",
		}, -- [59]
		{
			"hoping they have good hunter racials", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			133, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539195733,
			[50] = "CHAT_MSG_SAY",
		}, -- [60]
		{
			"rogue is ok but BM hunter >>>>>>>>> all", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			136, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539195759,
			[50] = "CHAT_MSG_SAY",
		}, -- [61]
		{
			"ya the silence is nuts in pvp", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			137, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539195769,
			[50] = "CHAT_MSG_SAY",
		}, -- [62]
		{
			"cool will do", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			140, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539195826,
			[50] = "CHAT_MSG_SAY",
		}, -- [63]
		{
			"see ya man have a good one", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			142, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539195865,
			[50] = "CHAT_MSG_SAY",
		}, -- [64]
		{
			"dude ", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			157, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196168,
			[50] = "CHAT_MSG_SAY",
		}, -- [65]
		{
			"check out the book", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			158, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196172,
			[50] = "CHAT_MSG_SAY",
		}, -- [66]
		{
			"Children of Time", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			159, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196174,
			[50] = "CHAT_MSG_SAY",
		}, -- [67]
		{
			"all about sentient spiders", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			161, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196178,
			[50] = "CHAT_MSG_SAY",
		}, -- [68]
		{
			"fuck me its scary", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			162, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196182,
			[50] = "CHAT_MSG_SAY",
		}, -- [69]
		{
			"ha no", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			164, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196213,
			[50] = "CHAT_MSG_SAY",
		}, -- [70]
		{
			"looking now", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			165, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196215,
			[50] = "CHAT_MSG_SAY",
		}, -- [71]
		{
			"holy shit", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			166, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196229,
			[50] = "CHAT_MSG_SAY",
		}, -- [72]
		{
			"goddamn the future is now", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			169, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196240,
			[50] = "CHAT_MSG_SAY",
		}, -- [73]
		{
			"crazy man", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			185, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196290,
			[50] = "CHAT_MSG_SAY",
		}, -- [74]
		{
			"cant wait to see one of those peeking in my window", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			188, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196302,
			[50] = "CHAT_MSG_SAY",
		}, -- [75]
		{
			"theres a new one?", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			192, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196367,
			[50] = "CHAT_MSG_SAY",
		}, -- [76]
		{
			"Im halfway saving to the caravan one ya", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			194, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196384,
			[50] = "CHAT_MSG_SAY",
		}, -- [77]
		{
			"ya gronns way bigger", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			196, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196407,
			[50] = "CHAT_MSG_SAY",
		}, -- [78]
		{
			"even for a lil belf", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			198, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196426,
			[50] = "CHAT_MSG_SAY",
		}, -- [79]
		{
			"ya I wonder who its for tbh like I play the AH but my AH alt is parked in the AH 24/7", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			201, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196501,
			[50] = "CHAT_MSG_SAY",
		}, -- [80]
		{
			"I guess for showing off your gold same as this thing lol", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			205, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196516,
			[50] = "CHAT_MSG_SAY",
		}, -- [81]
		{
			"although a widow mount for a sub rogue is pretty badass imo", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			206, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196534,
			[50] = "CHAT_MSG_SAY",
		}, -- [82]
		{
			"esp with these new warfront mogs", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			207, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196551,
			[50] = "CHAT_MSG_SAY",
		}, -- [83]
		{
			"ya its v cool looking ", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			209, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196566,
			[50] = "CHAT_MSG_SAY",
		}, -- [84]
		{
			"<LoFi> Small laid-back Raiding guild composed of close knit group looking to add to our core raiding team to progress Mythic+ Heroic/Mythic Raids Tues/Wed 6pm server, Willing to gear skilled players who show up", -- [1]
			"Rawrep-Kel'Thuzad", -- [2]
			"", -- [3]
			"2. Trade - City", -- [4]
			"Rawrep", -- [5]
			"", -- [6]
			2, -- [7]
			2, -- [8]
			"Trade - City", -- [9]
			0, -- [10]
			211, -- [11]
			"Player-3693-0A056D09", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cffff7c0aRawrep|r",
			[51] = 1539196579,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [85]
		{
			"scared em LOL", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			212, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196614,
			[50] = "CHAT_MSG_SAY",
		}, -- [86]
		{
			"gloomhound a good size too", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			213, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539196658,
			[50] = "CHAT_MSG_SAY",
		}, -- [87]
		{
			"woot |cffa335ee|Hitem:161381::153708::::::120:261::3:4:4798:1808:1507:4784:::|h[Permafrost-Encrusted Heart]|h|r", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"2. Trade - City", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			2, -- [7]
			2, -- [8]
			"Trade - City", -- [9]
			0, -- [10]
			4, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539199280,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [88]
		{
			"shame about the vers", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"2. Trade - City", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			2, -- [7]
			2, -- [8]
			"Trade - City", -- [9]
			0, -- [10]
			6, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539199319,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [89]
		{
			"but Ill take it", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"2. Trade - City", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			2, -- [7]
			2, -- [8]
			"Trade - City", -- [9]
			0, -- [10]
			7, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539199342,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [90]
		{
			"turns his |Hitem:118427|h|cFFFFFFFF[Autographed Hearthstone Card]|r|h card over, revealing a |cFFFFFFFF[Common Hearthstone Card]|r!", -- [1]
			"Thunderdeemz-Shadowmoon", -- [2]
			"", -- [3]
			"", -- [4]
			"Thunderdeemz-Shadowmoon", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			370, -- [11]
			"Player-154-091B592C", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffa330c9Thunderdeemz-Shadowmoon|r",
			[51] = 1539294968,
			[50] = "CHAT_MSG_EMOTE",
		}, -- [91]
		{
			"WTB BFA Greens 100g each, PST or COD!!!", -- [1]
			"Yeslesso-Stormreaver", -- [2]
			"", -- [3]
			"1. General - Orgrimmar", -- [4]
			"Yeslesso-Stormreaver", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Orgrimmar", -- [9]
			0, -- [10]
			563, -- [11]
			"Player-58-0B1D4C17", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cffaad372Yeslesso-Stormreaver|r",
			[51] = 1539296858,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [92]
		{
			"nice mount man", -- [1]
			"Kezt-Stormreaver", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Kezt-Stormreaver", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			30, -- [11]
			"Player-58-083D386E", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffaad372Kezt-Stormreaver|r",
			[51] = 1539350767,
			[50] = "CHAT_MSG_WHISPER_INFORM",
		}, -- [93]
		{
			"thanks", -- [1]
			"Kezt-Stormreaver", -- [2]
			"", -- [3]
			"", -- [4]
			"Kezt-Stormreaver", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			34, -- [11]
			"Player-58-083D386E", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffaad372Kezt-Stormreaver|r",
			[51] = 1539350773,
			[50] = "CHAT_MSG_WHISPER",
		}, -- [94]
		{
			"my hunter just picked up the matching blue pterrodax from the un-instanced atal'dzar or whatever", -- [1]
			"Kezt-Stormreaver", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Kezt-Stormreaver", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			46, -- [11]
			"Player-58-083D386E", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffaad372Kezt-Stormreaver|r",
			[51] = 1539350799,
			[50] = "CHAT_MSG_WHISPER_INFORM",
		}, -- [95]
		{
			"check it out, it's a sweet combo and pterros have updraft, useful sometimes", -- [1]
			"Kezt-Stormreaver", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Kezt-Stormreaver", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			47, -- [11]
			"Player-58-083D386E", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffaad372Kezt-Stormreaver|r",
			[51] = 1539350812,
			[50] = "CHAT_MSG_WHISPER_INFORM",
		}, -- [96]
		{
			"thats cool, i will", -- [1]
			"Kezt-Stormreaver", -- [2]
			"", -- [3]
			"", -- [4]
			"Kezt-Stormreaver", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			48, -- [11]
			"Player-58-083D386E", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffaad372Kezt-Stormreaver|r",
			[51] = 1539350833,
			[50] = "CHAT_MSG_WHISPER",
		}, -- [97]
		{
			"4 different colours iirc all flying around the uninstanced version of the dungeon, might take a bit to find a blue, iirc if you kill them the colours respawn randomly", -- [1]
			"Kezt-Stormreaver", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Kezt-Stormreaver", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			49, -- [11]
			"Player-58-083D386E", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffaad372Kezt-Stormreaver|r",
			[51] = 1539350873,
			[50] = "CHAT_MSG_WHISPER_INFORM",
		}, -- [98]
		{
			"such a nice mount", -- [1]
			"Acinonyx-Stormreaver", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Acinonyx-Stormreaver", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			55, -- [11]
			"Player-58-01A8C688", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffff7c0aAcinonyx-Stormreaver|r",
			[51] = 1539564535,
			[50] = "CHAT_MSG_WHISPER_INFORM",
		}, -- [99]
		{
			"big too", -- [1]
			"Acinonyx-Stormreaver", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Acinonyx-Stormreaver", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			56, -- [11]
			"Player-58-01A8C688", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffff7c0aAcinonyx-Stormreaver|r",
			[51] = 1539564537,
			[50] = "CHAT_MSG_WHISPER_INFORM",
		}, -- [100]
		{
			"yeah its a rep mount for new zone", -- [1]
			"Acinonyx-Stormreaver", -- [2]
			"", -- [3]
			"", -- [4]
			"Acinonyx-Stormreaver", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			57, -- [11]
			"Player-58-01A8C688", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffff7c0aAcinonyx-Stormreaver|r",
			[51] = 1539564547,
			[50] = "CHAT_MSG_WHISPER",
		}, -- [101]
		{
			"ya zandalar, I got the rep but havent paid for it yet lol", -- [1]
			"Acinonyx-Stormreaver", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Acinonyx-Stormreaver", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			65, -- [11]
			"Player-58-01A8C688", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffff7c0aAcinonyx-Stormreaver|r",
			[51] = 1539564572,
			[50] = "CHAT_MSG_WHISPER_INFORM",
		}, -- [102]
		{
			"wheres that mount from?", -- [1]
			"Fadedscourge-DemonSoul", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Fadedscourge-DemonSoul", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			105, -- [11]
			"Player-157-091666B5", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Fadedscourge-DemonSoul|r",
			[51] = 1539564609,
			[50] = "CHAT_MSG_WHISPER_INFORM",
		}, -- [103]
		{
			"|cffffff00|Hachievement:12812:Player-157-091666B5:1:10:12:18:4294967295:4294967295:4294967295:4294967295|h[Glory of the Wartorn Hero]|h|r", -- [1]
			"Fadedscourge-DemonSoul", -- [2]
			"", -- [3]
			"", -- [4]
			"Fadedscourge-DemonSoul", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			114, -- [11]
			"Player-157-091666B5", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Fadedscourge-DemonSoul|r",
			[51] = 1539564630,
			[50] = "CHAT_MSG_WHISPER",
		}, -- [104]
		{
			"oh nice ", -- [1]
			"Fadedscourge-DemonSoul", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Fadedscourge-DemonSoul", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			115, -- [11]
			"Player-157-091666B5", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Fadedscourge-DemonSoul|r",
			[51] = 1539564641,
			[50] = "CHAT_MSG_WHISPER_INFORM",
		}, -- [105]
		{
			"warmode on?", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			333, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539706239,
			[50] = "CHAT_MSG_RAID",
		}, -- [106]
		{
			"yeah", -- [1]
			"Asúrà-Area52", -- [2]
			"", -- [3]
			"", -- [4]
			"Asúrà-Area52", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			336, -- [11]
			"Player-3676-0A0B556F", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffc69b6dAsúrà-Area52|r",
			[51] = 1539706243,
			[50] = "CHAT_MSG_RAID",
		}, -- [107]
		{
			"you need to convert to a raid?", -- [1]
			"Sadrin-Medivh", -- [2]
			"", -- [3]
			"", -- [4]
			"Sadrin-Medivh", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			341, -- [11]
			"Player-62-003E5316", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cff8787edSadrin-Medivh|r",
			[51] = 1539706300,
			[50] = "CHAT_MSG_PARTY",
		}, -- [108]
		{
			"lets do lion's roar ppl", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"1. General - Arathi Highlands", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Arathi Highlands", -- [9]
			0, -- [10]
			347, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539706377,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [109]
		{
			"need a tank then g2g", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"1. General - Arathi Highlands", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			1, -- [7]
			1, -- [8]
			"General - Arathi Highlands", -- [9]
			0, -- [10]
			357, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			true, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539706482,
			[50] = "CHAT_MSG_CHANNEL",
		}, -- [110]
		{
			"we gonna need some heals", -- [1]
			"Khregnar-Stormreaver", -- [2]
			"", -- [3]
			"", -- [4]
			"Khregnar-Stormreaver", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			362, -- [11]
			"Player-58-097B3C9A", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffc69b6dKhregnar-Stormreaver|r",
			[51] = 1539706530,
			[50] = "CHAT_MSG_RAID_LEADER",
		}, -- [111]
		{
			"rdy", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			363, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539706531,
			[50] = "CHAT_MSG_RAID",
		}, -- [112]
		{
			"oh we had heals, guess they left", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			366, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539706547,
			[50] = "CHAT_MSG_RAID",
		}, -- [113]
		{
			"holds his |Hitem:64383|h|cff0070dd[Kaldorei Wind Chimes]|r|h to the wind.", -- [1]
			"Catapult-Mal'Ganis", -- [2]
			"", -- [3]
			"", -- [4]
			"Catapult-Mal'Ganis", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			370, -- [11]
			"Player-3684-09BF4001", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffff7c0aCatapult-Mal'Ganis|r",
			[51] = 1539706637,
			[50] = "CHAT_MSG_EMOTE",
		}, -- [114]
		{
			"no loot?", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			383, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1539706714,
			[50] = "CHAT_MSG_RAID",
		}, -- [115]
		{
			"This one isn't a weekly reset", -- [1]
			"Khregnar-Stormreaver", -- [2]
			"", -- [3]
			"", -- [4]
			"Khregnar-Stormreaver", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			386, -- [11]
			"Player-58-097B3C9A", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffc69b6dKhregnar-Stormreaver|r",
			[51] = 1539706725,
			[50] = "CHAT_MSG_RAID_LEADER",
		}, -- [116]
		{
			"Only resets each time control switches", -- [1]
			"Khregnar-Stormreaver", -- [2]
			"", -- [3]
			"", -- [4]
			"Khregnar-Stormreaver", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			388, -- [11]
			"Player-58-097B3C9A", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffc69b6dKhregnar-Stormreaver|r",
			[51] = 1539706730,
			[50] = "CHAT_MSG_RAID_LEADER",
		}, -- [117]
		{
			"Interrupted Veteran Sorceress's |cff71d5ff|Hspell:262007:0|h[Polymorph]|h|r!", -- [1]
			"Zaravià-BleedingHollow", -- [2]
			"", -- [3]
			"", -- [4]
			"Zaravià-BleedingHollow", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			223, -- [11]
			"Player-73-0BB8D86B", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffaad372Zaravià-BleedingHollow|r",
			[51] = 1540587725,
			[50] = "CHAT_MSG_INSTANCE_CHAT",
		}, -- [118]
		{
			"Interrupted Valorcall Spellweaver's |cff71d5ff|Hspell:262007:0|h[Polymorph]|h|r!", -- [1]
			"Zaravià-BleedingHollow", -- [2]
			"", -- [3]
			"", -- [4]
			"Zaravià-BleedingHollow", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			255, -- [11]
			"Player-73-0BB8D86B", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffaad372Zaravià-BleedingHollow|r",
			[51] = 1540587957,
			[50] = "CHAT_MSG_INSTANCE_CHAT",
		}, -- [119]
		{
			"wtf is this", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"Orcish", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			261, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1540588005,
			[50] = "CHAT_MSG_SAY",
		}, -- [120]
		{
			"Interrupted Valorcall Faithful's |cff71d5ff|Hspell:262309:0|h[Heal]|h|r!", -- [1]
			"Zaravià-BleedingHollow", -- [2]
			"", -- [3]
			"", -- [4]
			"Zaravià-BleedingHollow", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			277, -- [11]
			"Player-73-0BB8D86B", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffaad372Zaravià-BleedingHollow|r",
			[51] = 1540588222,
			[50] = "CHAT_MSG_INSTANCE_CHAT",
		}, -- [121]
		{
			"Interrupted High Sorceress Marala's |cff71d5ff|Hspell:277828:0|h[Arcane Blast]|h|r!", -- [1]
			"Zaravià-BleedingHollow", -- [2]
			"", -- [3]
			"", -- [4]
			"Zaravià-BleedingHollow", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			287, -- [11]
			"Player-73-0BB8D86B", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffaad372Zaravià-BleedingHollow|r",
			[51] = 1540588334,
			[50] = "CHAT_MSG_INSTANCE_CHAT",
		}, -- [122]
		{
			"NEED MORE AT CIRCLE", -- [1]
			"Dbcooper-Eldre'Thalas", -- [2]
			"", -- [3]
			"", -- [4]
			"Dbcooper-Eldre'Thalas", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			288, -- [11]
			"Player-123-04DA0E73", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffff7c0aDbcooper-Eldre'Thalas|r",
			[51] = 1540588378,
			[50] = "CHAT_MSG_INSTANCE_CHAT",
		}, -- [123]
		{
			"Interrupted High Sorceress Marala's |cff71d5ff|Hspell:277828:0|h[Arcane Blast]|h|r!", -- [1]
			"Zaravià-BleedingHollow", -- [2]
			"", -- [3]
			"", -- [4]
			"Zaravià-BleedingHollow", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			289, -- [11]
			"Player-73-0BB8D86B", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffaad372Zaravià-BleedingHollow|r",
			[51] = 1540588380,
			[50] = "CHAT_MSG_INSTANCE_CHAT",
		}, -- [124]
		{
			"Interrupted Captain Nials's |cff71d5ff|Hspell:272423:0|h[Summon League of Arathor]|h|r!", -- [1]
			"Zaravià-BleedingHollow", -- [2]
			"", -- [3]
			"", -- [4]
			"Zaravià-BleedingHollow", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			306, -- [11]
			"Player-73-0BB8D86B", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cffaad372Zaravià-BleedingHollow|r",
			[51] = 1540588661,
			[50] = "CHAT_MSG_INSTANCE_CHAT",
		}, -- [125]
		{
			"all in", -- [1]
			"Mekmonk-Mal'Ganis", -- [2]
			"", -- [3]
			"", -- [4]
			"Mekmonk-Mal'Ganis", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			309, -- [11]
			"Player-3684-09D8EB29", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cff00ff96Mekmonk-Mal'Ganis|r",
			[51] = 1540588691,
			[50] = "CHAT_MSG_INSTANCE_CHAT",
		}, -- [126]
		{
			"%s has earned the achievement |cffffff00|Hachievement:12879:Player-3693-09ED9289:1:10:26:18:4294967295:4294967295:4294967295:4294967295|h[Tour of War]|h|r!", -- [1]
			"Alorannah-Kel'Thuzad", -- [2]
			"", -- [3]
			"", -- [4]
			"Alorannah", -- [5]
			"", -- [6]
			0, -- [7]
			0, -- [8]
			"", -- [9]
			0, -- [10]
			317, -- [11]
			"Player-3693-09ED9289", -- [12]
			0, -- [13]
			false, -- [14]
			false, -- [15]
			false, -- [16]
			false, -- [17]
			[52] = "|cfffff468Alorannah|r",
			[51] = 1540588769,
			[50] = "CHAT_MSG_GUILD_ACHIEVEMENT",
		}, -- [127]
	},
}
